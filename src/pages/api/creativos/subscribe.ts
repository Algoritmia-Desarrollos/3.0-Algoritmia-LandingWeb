...
import { authenticateRequest, getAdminClient, json } from '../../../lib/creativos-app/server';

export const prerender = false;

const plans = {
	creator: { env: 'MERCADO_PAGO_PLAN_CREATOR_ID', fallback: 'MERCADO_PAGO_PLAN_ID', credits: 40, reason: 'Algoritmia Creative Studio — Creator' },
	pro: { env: 'MERCADO_PAGO_PLAN_PRO_ID', credits: 120, reason: 'Algoritmia Creative Studio — Pro' },
	scale: { env: 'MERCADO_PAGO_PLAN_SCALE_ID', credits: 300, reason: 'Algoritmia Creative Studio — Scale' },
} as const;
...
export const POST: APIRoute = async ({ request, url }) => {
	...
	const response = await fetch('https://api.mercadopago.com/preapproval', {
		...
		body: JSON.stringify({
			...
			reason: plan.reason,
			back_url: `${siteUrl}/creativos/app?subscription=return`,
		}),
	});
