...
import { catalogTaxonomy, creativeCatalog, creativeNumber, mapTemplateRecord, referencePresets, ringMeta, templatePath } from '../../lib/creativos-app/catalog';
import { isSupabaseConfigured, supabase } from '../../lib/creativos-app/supabase-browser';
...
const PROFILE_KEY = 'algoritmia-creative-profile-v1';
const SESSION_KEY = 'algoritmia-creative-session-v1';
const HISTORY_KEY = 'algoritmia-creative-history-v1';
const PRODUCTS_KEY = 'algoritmia-creative-products-v1';
const FAVORITES_KEY = 'algoritmia-creative-favorites-v1';
...
const subscriptionPlans = [
	{ code: 'creator', name: 'Creator', price: 19, credits: 40, description: 'Para empezar a producir con constancia.', featured: false },
	{ code: 'pro', name: 'Pro', price: 39, credits: 120, description: 'Para marcas que testean todas las semanas.', featured: true },
	{ code: 'scale', name: 'Scale', price: 79, credits: 300, description: 'Para equipos y cuentas con alto volumen.', featured: false },
];
...
function Moki({ className = '', label = 'Moki, el asistente creativo de Algoritmia' }: { className?: string; label?: string }) {
	return <span className={`moki-character ${className}`} role="img" aria-label={label}>
		<img className="moki-character-open" src="/images/creativos/moki-mascot.webp" alt="" aria-hidden="true"/>
		<img className="moki-character-blink" src="/images/creativos/moki-mascot-blink.webp" alt="" aria-hidden="true"/>
	</span>;
}
...
export default function CreativeApp() {
	...
	useEffect(() => {
		...
		async function loadAccount() {
			if (isSupabaseConfigured && supabase) {
				...
				const { data: templateRecords } = await supabase.from('creative_templates').select('*').eq('is_active', true).order('sort_order', { ascending: true });
				...
				if (records?.length) {
					const signed = await Promise.all(records.map(async (record) => {
						const { data: url } = await supabase.storage.from('creative-assets').createSignedUrl(record.output_path, 3600);
	...
	const navItems: Array<{ id: View; label: string; icon: string }> = [
		...
		{ id: 'studio', label: 'Estudio IA', icon: 'spark' },
		{ id: 'history', label: 'Mis creativos', icon: 'history' },
	...
	return (
		<div className="creative-app-shell">
			...
			<aside className={`studio-sidebar ${mobileMenu ? 'is-open' : ''}`}>
				<a className="studio-logo" href="/creativos/" aria-label="Volver a Algoritmia">
					<span><img src="/logoAlgoritmia.webp" alt=""/></span>
					<div><strong>Algoritmia</strong><small>Creative Studio</small></div>
				</a>
			...
			<main className="studio-main">
				<header className="studio-topbar">
					...
					<div className="studio-top-brand"><img src="/logoAlgoritmia.webp" alt=""/><strong>Creative Studio</strong></div>
...
function AuthScreen({ onSession }: { onSession: (session: AppSession) => void }) {
	...
	async function submit(event: FormEvent) {
		...
		try {
			if (isSupabaseConfigured && supabase) {
				if (mode === 'signup') {
					const { data, error: authError } = await supabase.auth.signUp({ email, password, options: { data: { full_name: fullName }, emailRedirectTo: `${window.location.origin}/creativos/app/` } });
				...
				const demoSession: DemoSession = { user: { id: 'demo-user', email: email || 'demo@algoritmia.app' } };
	...
	function enterDemo() {
		const demoSession: DemoSession = { user: { id: 'demo-user', email: 'demo@algoritmia.app' } };
		saveLocal(SESSION_KEY, demoSession);
		onSession(demoSession);
	}
	...
	async function signInWithGoogle() {
		setError(''); setNotice('');
		if (!isSupabaseConfigured || !supabase) {
			setError('Google se habilita al cargar las credenciales de Supabase y activar el proveedor en Auth.');
			return;
		}
		setLoading(true);
		try {
			const { error: authError } = await supabase.auth.signInWithOAuth({
				provider: 'google',
				options: { redirectTo: `${window.location.origin}/creativos/app/` },
			});
			if (authError) throw authError;
		} catch (cause) {
	...
	return (
		<div className="studio-auth">
			<a href="/creativos/" className="studio-auth-logo"><span><img src="/logoAlgoritmia.webp" alt=""/></span><strong>Algoritmia <small>Creative Studio</small></strong></a>
			<section className="studio-auth-panel">
				<div className="studio-auth-card">
					<div className="studio-auth-heading">
						...
						<h2>{authStep === 'email' ? (mode === 'signup' ? 'Creá tu primera imagen en minutos' : 'Volvé a tu estudio creativo') : (mode === 'signup' ? 'Terminemos de crear tu cuenta' : 'Ingresá tu contraseña')}</h2>
					...
					{authStep === 'email' && <><button className="studio-google-button" type="button" onClick={signInWithGoogle} disabled={loading}><img src="/images/creativos/google-g.svg" alt=""/>{mode === 'signup' ? 'Registrarse con Google' : 'Ingresar con Google'}</button><div className="studio-auth-divider"><span>o</span></div></>}
					<form onSubmit={submit}>
						...
						<button className="studio-primary-button" disabled={loading || (authStep === 'email' && !email.trim())}>{loading ? <span className="studio-spinner small"/> : <>{authStep === 'email' ? 'Continuar' : mode === 'signup' ? 'Crear cuenta y probar gratis' : 'Ingresar'}<Icon name="arrow" size={18}/></>}</button>
					</form>
				...
				<p className="studio-auth-switch">{mode === 'signup' ? '¿Ya tenés una cuenta?' : '¿Todavía no tenés cuenta?'} <button onClick={() => changeMode(mode === 'signup' ? 'login' : 'signup')}>{mode === 'signup' ? 'Iniciar sesión' : 'Crear una gratis'}</button></p>
			</section>
			<div className="studio-auth-moki" aria-hidden="true"><span>¡Hola! Te acompaño.</span><img src="/images/creativos/moki-mascot.webp" alt=""/></div>
		</div>
	);
}
...
function Onboarding({ profile, email, onSave }: { profile: AppProfile; email: string; onSave: (profile: AppProfile, logo?: File | null) => Promise<void> }) {
	...
	return <div className="studio-onboarding">
		<header><a href="/creativos/"><img src="/logoAlgoritmia.webp" alt=""/><strong>Algoritmia</strong></a><span>Paso 1 de 1</span></header>
		<main>
			<div className="studio-onboarding-intro"><span><Icon name="spark"/></span><p>CONFIGURÁ TU MARCA</p><h1>Una vez. Después,<br/>todo sale con tu identidad.</h1><small>Analizamos tu web y tu Instagram para preparar el catálogo y el contexto de la IA.</small></div>
			<div className="studio-onboarding-visual"><Moki className="studio-onboarding-moki"/><div><span><i/> MOKI ESTÁ ANALIZANDO</span><strong>Tu catálogo se prepara solo.</strong><small>La IA detecta productos, colores y contexto de marca.</small><div className="onboarding-source-flow"><b>tumarca.com</b><i/><b>24 productos listos</b></div></div></div>
			<form onSubmit={submit}>
				<div className="studio-form-grid">
					...
					<label>Sitio web<input type="url" value={draft.website} onChange={(e) => setDraft({ ...draft, website: e.target.value })} placeholder="https://tumarca.com" /></label>
					<label>Instagram o URL<input value={draft.instagram} onChange={(e) => setDraft({ ...draft, instagram: e.target.value })} placeholder="https://instagram.com/tumarca" /></label>
				...
				{error && <p className="studio-form-error">{error}</p>}
				<button className="studio-primary-button" disabled={saving}>{saving ? <><span className="studio-spinner small"/>Analizando tu negocio…</> : <>Guardar y entrar al estudio<Icon name="arrow" size={18}/></>}</button>
			</form>
		</main>
	</div>;
}
...
function Dashboard({ profile, email, history, catalog, favorites, onView, onChoose, onReuse }: { profile: AppProfile; email: string; history: Generation[]; catalog: Creativo[]; favorites: Set<number>; onView: (view: View) => void; onChoose: (creative: Creativo) => void; onReuse: (generation: Generation) => void }) {
	...
	return <>
		<div className="studio-page-heading"><div><p>INICIO</p><h1>Buen día, {firstName(profile, email)}.</h1><span>¿Qué creativo ganador vamos a encontrar hoy?</span></div><button className="studio-primary-button compact" onClick={() => onView('library')}><Icon name="plus" size={17}/>Crear creativo</button></div>
		<section className="studio-hero-card">
			<div className="studio-hero-copy"><span className="studio-eyebrow light"><i/><span>TU ESTUDIO ESTÁ LISTO</span></span><h2>Encontrá una idea.<br/>Convertirla en una imagen <em>es lo fácil.</em></h2><p>Buscá por objetivo, formato o estilo. Tu marca y tus productos ya quedan listos para cada generación.</p><button onClick={() => onView('library')}>Explorar la biblioteca <Icon name="arrow" size={17}/></button></div>
			<div className="studio-hero-visual studio-hero-render"><Moki className="studio-dashboard-moki"/><span className="hero-render-chip top"><i/>Moki está listo</span><span className="hero-render-chip bottom"><b>Nuevas</b> ideas cada semana</span><span className="hero-render-chip middle">{favorites.size} guardadas</span><div className="hero-spark"><Icon name="spark"/></div></div>
		</section>
		<div className="studio-section-title"><div><h2>Ideas para empezar</h2><p>Opciones claras para crear tu primera imagen.</p></div><button onClick={() => onView('library')}>Ver biblioteca <Icon name="arrow" size={15}/></button></div>
		...
		<section className="studio-flow-strip"><div><span>1</span><p><strong>Elegí una idea</strong><small>Biblioteca organizada por objetivo</small></p></div><i/><div><span>2</span><p><strong>Sumá tu producto</strong><small>O generá una oferta general</small></p></div><i/><div><span>3</span><p><strong>Generá y publicá</strong><small>Tu marca queda precargada</small></p></div></section>
		{history.length > 0 && <><div className="studio-section-title"><div><h2>Tus últimos creativos</h2><p>Descargalos o volvé a usar la misma configuración.</p></div><button onClick={() => onView('history')}>Ver todos <Icon name="arrow" size={15}/></button></div><div className="studio-recent-row">{history.slice(0, 4).map((item) => <GenerationCard key={item.id} item={item} onReuse={() => onReuse(item)}/>)}</div></>}
	</>;
}
...
function CreativeFeatureCard({ creative, index, onClick }: { creative: Creativo; index: number; onClick: () => void }) {
	return <button className={`studio-feature-card variant-${index + 1}`} onClick={onClick}>
		...
		<div className="studio-feature-info"><span>#{creativeNumber(creative.id)} · {creative.n}</span><h3>{creative.nombre}</h3><p>{creative.cuando}</p><footer>Usar este ángulo <Icon name="arrow" size={15}/></footer></div>
	</button>;
}
...
function Library({ items, favorites, onChoose, onToggleFavorite }: { items: Creativo[]; favorites: Set<number>; onChoose: (creative: Creativo) => void; onToggleFavorite: (id: number) => void }) {
	...
	return <>
		<div className="studio-page-heading"><div><p>BIBLIOTECA VIVA</p><h1>Encontrá la imagen que necesitás.</h1><span>Navegá por objetivo, categoría y tipo. La biblioteca crece cada semana.</span></div></div>
				...
				<div className="studio-library-grid">{filtered.map((creative) => {
					...
					return <article className="studio-library-card catalog-card-shell" key={creative.id} style={{ '--card-accent': path?.group.accent || meta?.accent } as React.CSSProperties}>
						...
						<button className="catalog-card-open" onClick={() => onChoose(creative)}>
							...
							<div className="library-card-copy"><small>{path?.leaf.label || meta?.label}</small><h3>{creative.nombre}</h3><p>{creative.sirve}</p><footer><span>Usar esta idea</span><Icon name="arrow" size={15}/></footer></div>
						</button>
					</article>;
				})}</div>
...
function ProductCatalog({ products, profile, onRefresh, onSync, onCreate }: { products: Product[]; profile: AppProfile; session: AppSession; onRefresh: () => Promise<Product[]>; onSync: () => Promise<void>; onCreate: () => void }) {
	...
	return <>
		<div className="studio-page-heading"><div><p>MIS PRODUCTOS</p><h1>Tu catálogo, listo para crear.</h1><span>Fotos y datos privados que la IA usa únicamente en tus anuncios.</span></div><button className="studio-primary-button compact" onClick={onCreate}><Icon name="plus" size={17}/>Crear creativo</button></div>
		<section className={`product-sync-card status-${profile.catalogStatus}`}><Moki className="product-sync-moki"/><div><span><Icon name={profile.catalogStatus === 'ready' ? 'check' : 'spark'} size={18}/></span><p><small>{statusLabel}</small><strong>{products.length} productos disponibles</strong><em>{profile.website || profile.instagram || 'Agregá una fuente desde Mi marca'}</em></p></div><button onClick={sync} disabled={syncing || (!profile.website && !profile.instagram)}>{syncing ? <><span className="studio-spinner small"/> Analizando…</> : 'Volver a analizar'}</button></section>
		...
		{filtered.length ? <div className="product-catalog-grid">{filtered.map((product) => <article key={product.id}><div>{product.imageUrl ? <img src={product.imageUrl} alt={product.name}/> : <span><Icon name="bag" size={30}/></span>}<small>{product.source === 'manual' ? 'Cargado por vos' : 'Desde tu tienda'}</small></div><footer><h3>{product.name}</h3><p>{product.description || 'Listo para usar en una generación.'}</p><span>{product.priceText ? `${product.priceText} ${product.currency}` : 'Sin precio público'}</span></footer></article>)}</div> : <div className="studio-empty large"><span><Icon name="bag"/></span><h3>No encontramos productos</h3><p>Sincronizá tu web o cargá una foto durante el paso a paso.</p><button onClick={sync}>Analizar mi negocio</button></div>}
	</>;
}
...
function Studio({ creative, reuseSeed, profile, session, products, onProductsChanged, onChooseLibrary, onGenerated, onToast }: { creative: Creativo; reuseSeed: Generation | null; profile: AppProfile; session: AppSession; products: Product[]; onProductsChanged: () => Promise<Product[]>; onChooseLibrary: () => void; onGenerated: (generation: Generation, credits: number) => void; onToast: (message: string) => void }) {
	...
	const typeOptions = [
		{ id: 'product', title: 'Producto protagonista', copy: 'El producto real domina la composición.', badge: 'Más usado', icon: 'bag' },
		{ id: 'promotion', title: 'Promoción general', copy: 'Oferta de marca con producto opcional.', badge: 'Sin producto', icon: 'spark' },
		{ id: 'lifestyle', title: 'Lifestyle', copy: 'El producto aparece dentro de una escena real.', badge: 'Contexto', icon: 'brand' },
		{ id: 'catalog', title: 'Catálogo premium', copy: 'Fondo limpio, luz controlada y foco comercial.', badge: 'Ecommerce', icon: 'grid' },
	];
	const formatOptions = [
	...
	useEffect(() => {
		...
		void (async () => {
			const { data } = await supabase.from('creative_references').select('id,name,image_path,prompt_notes').eq('template_id', creative.id).eq('is_active', true).order('sort_order').limit(5);
			if (!data?.length || cancelled) return;
			const loaded = (await Promise.all(data.map(async (item) => {
				const { data: signed } = await supabase.storage.from('creative-references').createSignedUrl(item.image_path, 3600);
	...
	async function generate(sourceGeneration: Generation | null = null) {
		...
		try {
			...
			onToast(isSupabaseConfigured ? 'Tu creativo está listo y guardado.' : 'Vista demo creada. Conectá las credenciales para producirla con IA.');
		} catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo generar el creativo.'); }
	...
	return <>
		...
		<section className="wizard-launch-card"><div className="wizard-launch-copy"><span><Icon name="spark"/> FLUJO GUIADO</span><h2>Todo lo necesario para generar,<br/><em>en el orden correcto.</em></h2><p>Producto, referencia, tipo de imagen, formato y dirección creativa. Tu marca ya está precargada.</p><button onClick={() => { setWizardOpen(true); if (result) setStep(6); }}>Abrir generador <Icon name="arrow" size={17}/></button></div><div className="wizard-launch-stack"><article><small>01 · ÁNGULO</small><strong>{creative.nombre}</strong><span>{meta?.label}</span></article><article><small>02 · CATÁLOGO</small><strong>{products.length} productos listos</strong><span>{profile.catalogStatus === 'ready' ? 'Sincronizado con tu negocio' : 'Carga manual disponible'}</span></article><article><small>03 · SALIDA</small><strong>4 formatos</strong><span>Feed, Stories y horizontal</span></article></div></section>
		<div className="studio-flow-strip wizard-flow"><div><span>1</span><p><strong>Elegí el tipo</strong><small>Producto, promo o lifestyle</small></p></div><i/><div><span>2</span><p><strong>Buscá el producto</strong><small>Desde tu catálogo privado</small></p></div><i/><div><span>3</span><p><strong>Definí la salida</strong><small>Referencia, formato y dirección</small></p></div></div>

		{wizardOpen && <div className="creative-wizard-overlay" role="dialog" aria-modal="true" aria-label="Generador guiado de creativos"><div className="creative-wizard-modal">
			<header className="wizard-header"><div><span className="wizard-brand-mark"><Icon name="spark" size={17}/></span><p><small>CREATIVE STUDIO</small><strong>{step === 6 ? 'Tu creativo está listo' : `Crear · ${creative.nombre}`}</strong></p></div><button onClick={() => setWizardOpen(false)} aria-label="Cerrar generador"><Icon name="close"/></button></header>
			{step <= 5 && <div className="wizard-progress">{['Tipo', 'Producto', 'Variante', 'Formato', 'Detalles'].map((label, index) => <button key={label} className={step === index + 1 ? 'active' : step > index + 1 ? 'done' : ''} onClick={() => index + 1 < step && setStep(index + 1)}><span>{step > index + 1 ? <Icon name="check" size={11}/> : index + 1}</span><b>{label}</b></button>)}</div>}
			<div className="wizard-body"><main>
				{step === 1 && <section className="wizard-step"><div className="wizard-step-heading"><span>PASO 1 DE 5</span><h2>¿Qué tipo de imagen buscás?</h2><p>Esto define el tratamiento visual; no cambia el ángulo ganador.</p></div><div className="wizard-type-grid">{typeOptions.map((item) => <button key={item.id} className={imageType === item.id ? 'active' : ''} onClick={() => setImageType(item.id)}><span><Icon name={item.icon}/></span><em>{item.badge}</em><h3>{item.title}</h3><p>{item.copy}</p>{imageType === item.id && <b><Icon name="check" size={13}/></b>}</button>)}</div></section>}
				{step === 2 && <section className="wizard-step"><div className="wizard-step-heading"><span>PASO 2 DE 5</span><h2>{imageType === 'promotion' ? '¿Querés sumar un producto?' : 'Elegí el producto protagonista'}</h2><p>Buscá entre los productos analizados o guardá una foto nueva.</p></div><label className="wizard-product-search"><Icon name="search" size={18}/><input aria-label="Buscar producto" value={productQuery} onChange={(event) => setProductQuery(event.target.value)} placeholder="Buscar producto…"/><span>{filteredProducts.length}</span></label>{imageType === 'promotion' && <button className={`wizard-no-product ${!selectedProductId ? 'active' : ''}`} onClick={() => setSelectedProductId('')}><span><Icon name="spark"/></span><p><strong>Promoción general de marca</strong><small>Generar sin elegir un producto puntual</small></p>{!selectedProductId && <b><Icon name="check" size={13}/></b>}</button>}<div className="wizard-product-grid">{filteredProducts.map((product) => <button key={product.id} className={selectedProductId === product.id ? 'active' : ''} onClick={() => setSelectedProductId(product.id)}><div>{product.imageUrl ? <img src={product.imageUrl} alt={product.name}/> : <span><Icon name="bag"/></span>}{selectedProductId === product.id && <b><Icon name="check" size={13}/></b>}</div><strong>{product.name}</strong><small>{product.priceText ? `${product.priceText} ${product.currency}` : product.source === 'manual' ? 'Carga manual' : 'Desde tu tienda'}</small></button>)}</div><div className="wizard-upload-product"><input ref={uploadInput} hidden type="file" accept="image/png,image/jpeg,image/webp,image/avif" onChange={(event) => setUploadFile(event.target.files?.[0] || null)}/>{uploadFile ? <><img src={uploadPreview} alt="Producto nuevo"/><label>Nombre del producto<input value={uploadName} onChange={(event) => setUploadName(event.target.value)} placeholder="Ej. Zapatilla Urban White"/></label><button onClick={saveUploadedProduct} disabled={uploading}>{uploading ? 'Guardando…' : 'Guardar en mi catálogo'}</button></> : <button onClick={() => uploadInput.current?.click()}><span><Icon name="upload"/></span><p><strong>¿No aparece?</strong><small>Subí una foto y queda guardada en Supabase.</small></p><b>Cargar producto</b></button>}</div></section>}
				{step === 3 && <section className="wizard-step"><div className="wizard-step-heading"><span>PASO 3 DE 5</span><h2>Elegí la variante visual</h2><p>{references.length ? 'Estas referencias reales están asociadas al ángulo elegido.' : 'Elegí cuánto querés priorizar la composición, el impacto o la marca.'}</p></div>{references.length ? <div className="wizard-reference-grid">{references.map((item, index) => <button key={item.id} className={referenceId === item.id ? 'active' : ''} onClick={() => setReferenceId(item.id)}><div><img src={item.imageUrl} alt={item.name}/><span>REF {String(index + 1).padStart(2, '0')}</span></div><strong>{item.name}</strong><small>{item.description}</small>{referenceId === item.id && <b><Icon name="check" size={13}/></b>}</button>)}</div> : <div className="wizard-variant-grid">{referencePresets.map((item, index) => <button key={item.id} className={preset === item.id ? 'active' : ''} onClick={() => setPreset(item.id)}><div className={`preset-preview preset-${index + 1}`}><i/><b/><span/><small/></div><em>{item.label}</em><strong>{item.name}</strong><p>{item.description}</p>{preset === item.id && <b><Icon name="check" size={13}/></b>}</button>)}</div>}</section>}
				...
				{step === 5 && <section className="wizard-step"><div className="wizard-step-heading"><span>PASO 5 DE 5 · OPCIONAL</span><h2>¿Querés agregar una indicación?</h2><p>Si lo dejás vacío, la IA se guía por tu página web, Instagram, identidad de marca y el producto elegido.</p></div><label className="wizard-brief"><textarea value={brief} maxLength={500} onChange={(event) => setBrief(event.target.value)} placeholder="Opcional. Ej: destacar envío gratis, usar un tono premium o dejar más aire."/><span>{brief.length}/500</span></label><div className="wizard-final-check"><span><Icon name="check" size={14}/></span><p><strong>No hace falta que escribas nada</strong><small>La información verificada de tu negocio y producto ya está precargada. La IA no inventa precios ni beneficios.</small></p></div></section>}
				{step === 6 && <section className="wizard-result"><div className={`wizard-result-image result-${format}`}>{generating ? <div><span className="studio-spinner"/><h3>Componiendo tu creativo…</h3></div> : result && <img src={result.imageUrl} alt={`Creativo ${result.title}`}/>}</div>{result && <div className="wizard-result-copy"><span><Icon name="check" size={14}/> GENERACIÓN COMPLETA</span><h2>Tu imagen está lista.</h2><p>Quedó guardada en “Mis creativos”. Podés descargarla o usarla como referencia para pedir un ajuste.</p><div className="wizard-result-actions"><a href={result.imageUrl} download={`algoritmia-${creative.id}.jpg`}><Icon name="download" size={18}/>Descargar</a><button onClick={() => { setResult(null); setRevisionBrief(''); setStep(1); }}><Icon name="plus" size={17}/>Nueva desde cero</button></div><div className="wizard-revision"><header><span><Icon name="spark" size={16}/></span><p><strong>Modificar esta imagen</strong><small>La nueva generación usará este resultado como referencia.</small></p></header><label>¿Qué querés cambiar?<textarea value={revisionBrief} maxLength={500} onChange={(event) => setRevisionBrief(event.target.value)} placeholder="Ej: reemplazá el fondo por uno claro y hacé más visible el beneficio."/></label>{products.length > 0 && <label>Producto de esta versión<select value={selectedProductId} onChange={(event) => setSelectedProductId(event.target.value)}>{imageType === 'promotion' && <option value="">Promoción sin producto</option>}{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label>}<div className="wizard-revision-strength">{([{ id: 'exact', title: 'Exactamente igual', copy: 'Solo cambia lo que pedís.' }, { id: 'light', title: 'Variar un poco', copy: 'Mismo diseño, pequeños cambios.' }, { id: 'strong', title: 'Cambiar mucho', copy: 'Nueva composición, mismo ángulo.' }] as { id: VariationStrength; title: string; copy: string }[]).map((option) => <button key={option.id} className={variationStrength === option.id ? 'active' : ''} onClick={() => setVariationStrength(option.id)}><span>{variationStrength === option.id && <Icon name="check" size={11}/>}</span><p><strong>{option.title}</strong><small>{option.copy}</small></p></button>)}</div><button className="wizard-revision-generate" onClick={() => void generate(result)} disabled={generating}><Icon name="spark" size={17}/>Generar ajuste <span>1 crédito</span></button></div></div>}</section>}
				{error && <p className="wizard-error">{error}</p>}
			</main>{step <= 5 && <aside className="wizard-summary"><small>TU SELECCIÓN</small><div><span style={{ background: meta?.accent }}>{creativeNumber(creative.id)}</span><p><strong>{creative.nombre}</strong><small>{meta?.label} · {creative.n}</small></p></div><ul><li><span>Tipo</span><b>{typeOptions.find((item) => item.id === imageType)?.title}</b></li><li><span>Producto</span><b>{selectedProduct?.name || (imageType === 'promotion' ? 'Promoción general' : 'Sin elegir')}</b></li><li><span>Variante</span><b>{currentVariant || 'Fiel al ganador'}</b></li><li><span>Formato</span><b>{formatOptions.find((item) => item.id === format)?.ratio}</b></li></ul><footer><span><Icon name="brand" size={15}/></span><p><strong>{profile.brandName}</strong><small>Web, Instagram y colores precargados</small></p></footer></aside>}</div>
			{step <= 5 && <footer className="wizard-footer"><button onClick={() => step === 1 ? setWizardOpen(false) : setStep(step - 1)}>{step === 1 ? 'Cancelar' : 'Atrás'}</button>{step < 5 ? <button className="primary" onClick={nextStep}>Continuar <Icon name="arrow" size={17}/></button> : <button className="primary generate" onClick={() => void generate()} disabled={generating}>{generating ? <><span className="studio-spinner small"/> Generando…</> : <><Icon name="spark" size={17}/>Generar creativo <span>1 crédito</span></>}</button>}</footer>}
		</div></div>}
	</>;
}
	...
	return <>
		...
		<div className="studio-editor-layout">
			...
			<aside className="studio-result-panel">
				...
				{result && <div className="studio-result-actions"><a href={result.imageUrl} download={`algoritmia-${creative.id}-${creative.nombre.toLowerCase().replace(/\s+/g, '-')}.jpg`}><Icon name="download" size={18}/>Descargar</a><button onClick={generate}><Icon name="spark" size={17}/>Generar variante</button></div>}
...
function History({ history, onCreate, onReuse }: { history: Generation[]; onCreate: () => void; onReuse: (item: Generation) => void }) {
	return <><div className="studio-page-heading"><div><p>MIS CREATIVOS</p><h1>Todo lo que ya generaste.</h1><span>Descargá un resultado o reutilizá su configuración con un toque.</span></div><button className="studio-primary-button compact" onClick={onCreate}><Icon name="plus" size={17}/>Crear nuevo</button></div>{history.length ? <div className="studio-history-grid">{history.map((item) => <GenerationCard key={item.id} item={item} onReuse={() => onReuse(item)}/>)}</div> : <div className="studio-empty large"><span><Icon name="history"/></span><h3>Todavía no generaste creativos</h3><p>Elegí una idea de la biblioteca y creá el primero.</p><button onClick={onCreate}>Explorar biblioteca</button></div>}</>;
}

function GenerationCard({ item, onReuse }: { item: Generation; onReuse?: () => void }) {
	return <article className="studio-generation-card"><div><img src={item.imageUrl} alt={item.title}/><a href={item.imageUrl} download aria-label={`Descargar ${item.title}`}><Icon name="download" size={17}/></a></div><footer><small>{item.category}</small><h3>{item.title}</h3><span>{new Intl.DateTimeFormat('es-AR', { day: '2-digit', month: 'short' }).format(new Date(item.createdAt))}</span>{onReuse && <button onClick={onReuse}><Icon name="history" size={14}/>Reutilizar configuración</button>}</footer></article>;
}
...
function Plans({ profile, session }: { profile: AppProfile; session: AppSession }) {
	...
	return <><div className="studio-page-heading"><div><p>PLANES</p><h1>Elegí cuánto querés crear.</h1><span>Todos incluyen la biblioteca completa, tu marca, productos, favoritos e historial.</span></div></div>
		...
		{error && <p className="studio-form-error">{error}</p>}
		<div className="studio-plans-grid">{subscriptionPlans.map((plan) => <article key={plan.code} className={plan.featured ? 'featured' : ''}>{plan.featured && <span>MÁS ELEGIDO</span>}<small>PLAN {plan.name.toUpperCase()}</small><h2><b>USD</b>{plan.price}<em>/mes</em></h2><p>{plan.description}</p><strong>{plan.credits} generaciones por mes</strong><ul><li><Icon name="check" size={14}/>Biblioteca completa y nuevas categorías</li><li><Icon name="check" size={14}/>Marca y productos precargados</li><li><Icon name="check" size={14}/>Favoritos, historial y reutilización</li></ul><button onClick={() => subscribe(plan.code)} disabled={Boolean(billing) || (profile.subscriptionStatus === 'authorized' && profile.planCode === plan.code)}>{profile.subscriptionStatus === 'authorized' && profile.planCode === plan.code ? 'Plan actual' : billing === plan.code ? 'Abriendo pago…' : `Elegir ${plan.name}`}</button></article>)}</div>
...
function BrandSettings({ profile, onSave, onSync, session, onPlans }: { profile: AppProfile; onSave: (profile: AppProfile, logo?: File | null) => Promise<void>; onSync: () => Promise<void>; session: AppSession; onPlans: () => void }) {
	...
	return <><div className="studio-page-heading"><div><p>MI MARCA</p><h1>Tu identidad, siempre precargada.</h1><span>Editá una vez; la usamos como contexto en cada generación.</span></div></div><div className="studio-settings-layout"><form className="studio-settings-card" onSubmit={save}><header><span>{(draft.brandName || 'M').slice(0, 1).toUpperCase()}</span><div><h2>Identidad de marca</h2><p>Datos visibles y contexto de generación.</p></div></header><div className="studio-form-grid"><label className="wide">Nombre de la marca<input value={draft.brandName} onChange={(e) => setDraft({ ...draft, brandName: e.target.value })} required/></label><label>Tu nombre<input value={draft.fullName} onChange={(e) => setDraft({ ...draft, fullName: e.target.value })}/></label><label>Email<input value={getSessionEmail(session)} disabled/></label><label>Sitio web<input type="url" value={draft.website} onChange={(e) => setDraft({ ...draft, website: e.target.value })}/></label><label>Instagram<input value={draft.instagram} onChange={(e) => setDraft({ ...draft, instagram: e.target.value })}/></label><label>Color principal<span className="studio-color-input"><input type="color" value={draft.primaryColor} onChange={(e) => setDraft({ ...draft, primaryColor: e.target.value })}/><b>{draft.primaryColor}</b></span></label><label>Color de apoyo<span className="studio-color-input"><input type="color" value={draft.secondaryColor} onChange={(e) => setDraft({ ...draft, secondaryColor: e.target.value })}/><b>{draft.secondaryColor}</b></span></label><label className="wide studio-logo-upload"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => setLogo(e.target.files?.[0] || null)}/><span><Icon name="upload"/></span><div><strong>{logo ? logo.name : 'Actualizar logo'}</strong><small>Se conserva en tu biblioteca privada.</small></div><b>Elegir archivo</b></label></div>{error && <p className="studio-form-error">{error}</p>}<button className="studio-primary-button compact" disabled={saving}>{saving ? <span className="studio-spinner small"/> : 'Guardar cambios'}</button></form><aside className="studio-billing-card"><span className="studio-plan-orb"><Icon name="spark"/></span><small>{profile.subscriptionStatus === 'authorized' ? `PLAN ${profile.planCode.toUpperCase()}` : 'PRUEBA GRATUITA'}</small><h2>{profile.credits} {profile.credits === 1 ? 'generación disponible' : 'generaciones disponibles'}</h2><p>{profile.subscriptionStatus === 'authorized' ? `Tu plan incluye ${profile.monthlyCredits} generaciones mensuales.` : 'Tus 3 pruebas no vencen. Elegí un plan cuando quieras seguir creando.'}</p><ul><li><Icon name="check" size={14}/>Biblioteca en expansión</li><li><Icon name="check" size={14}/>Ideas guardadas e historial</li><li><Icon name="check" size={14}/>Marca y productos privados</li></ul><button onClick={onPlans}>Ver los tres planes<Icon name="arrow" size={16}/></button><footer>Pago seguro y recurrente con Mercado Pago.</footer></aside></div></>;
}
